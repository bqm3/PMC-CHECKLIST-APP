...
const AN_DAOTAO_SCREEN = ({ route, navigation }) => {
  ...
  const fetchBaoCaos = useCallback(
    async (page = 1, limit = 10) => {
      try {
        ...
        const response = await anDaoTaoAPI.getAllDaoTaoGiaoCa(params, authToken);

        if (page === 1) {
          setBaoCaos(response.data?.records || []);
        } else {
          setBaoCaos((prev) => [...prev, ...(response.data?.records || [])]);
        }
    ...
    [filters.tu_ngay, filters.den_ngay, filters.id_du_an, authToken, debouncedSearch]
  );
  ...
  // Reset page về 1 khi debounced search thay đổi
  useEffect(() => {
    setFilters((prev) => ({
      ...prev,
      page: 1,
  ...
  const handleViewDetail = (baoCao) => {
    navigation.navigate("an_ninh_dao_tao_form", {
      baoCaoId: baoCao.id,
      ma_du_an: baoCao.ma_du_an,
      mode: "edit",
    });
  };

  const handleCreateNew = () => {
    navigation.navigate("an_ninh_dao_tao_form", { mode: "create" });
  };
...
export default AN_DAOTAO_SCREEN;